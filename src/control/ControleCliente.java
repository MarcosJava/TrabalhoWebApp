...
public class ControleCliente extends HttpServlet {
	...
	protected void execute(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		try{
	        ...
	        /*switch case s� funciona a partir do Java 7*/
...
protected void cadastrar(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try{
			...
			if(new ClienteDao().cadastrar(cliente)){
				...
				request.setAttribute("msg", 
				"<div class='alert alert-danger'>Cliente j� possui possui um cadastro no sistema</div>");
			}
		}catch(Exception e){
			e.printStackTrace();
			request.setAttribute("msg", 
					"<div class='alert alert-danger'>Funcionario n�o cadastrado</div>");
		}finally{
...
protected void alterar(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
	...
	try{
		...
		if(new ClienteDao().alterar(cliente)){
			...
			request.setAttribute("msg", 
			"<div class='alert alert-danger'>CPF informado est� associado a outro cliente.</div>");
		}
	}catch(Exception e){
		e.printStackTrace();
		request.setAttribute("msg", 
